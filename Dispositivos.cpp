...
LRESULT CALLBACK DialogoInterfacesProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam) {
    ...
    switch (message) {
        case WM_CREATE: { //si la ventana se esta creando
            ...
            CreateWindowEx(0, "STATIC", "Selecciona la interfaz de red para capturar:", 
                        WS_VISIBLE | WS_CHILD, 15, 15, 350, 20, hwnd, NULL, NULL, NULL);

            hwndLista = CreateWindowEx(WS_EX_CLIENTEDGE, "LISTBOX", "", 
                                    WS_VISIBLE | WS_CHILD | LBS_NOTIFY | WS_VSCROLL | WS_BORDER, 
                                    15, 35, 450, 180, hwnd, (HMENU)LISTA, NULL, NULL);

            CreateWindowEx(0, "BUTTON", "Aceptar", //boton de acentar 
            ...
            if (pcap_findalldevs(&alldevs, error_buffer)==-1){ //busqueda de las interfaces
                MessageBox(hwnd, "Error al buscar interfaces de red", "Error", MB_ICONERROR);
                DestroyWindow(hwnd);
                return 0;
            }
            ...
            for (pcap_if_t *d=alldevs; d != NULL; d = d->next) {
                ...
                SendMessage(hwndLista, LB_ADDSTRING, 0, (LPARAM)texto); //agrega el texyo al combo
                SendMessage(hwndLista, LB_SETITEMDATA, index, (LPARAM)d);
                index++; //incrementa contador
            }

            if (index==0){ //si nunca avanzó mostrar que no habian interfaces disponibles
                MessageBox(hwnd, "No se encontraron interfaces disponibles", "Error", MB_ICONWARNING);
                pcap_freealldevs(alldevs);
                DestroyWindow(hwnd);
            } else {
                SendMessage(hwndLista, LB_SETCURSEL, 0, 0);
            }
            break;
        }
        case WM_COMMAND: { //interaccion del usuario
            ...
            if (LOWORD(wParam) == LISTA && HIWORD(wParam) == LBN_DBLCLK) {
                SendMessage(hwnd, WM_COMMAND, MAKEWPARAM(ID_BOTON_ACEPTAR, 0), 0);
            }

            if (LOWORD(wParam) == ID_BOTON_ACEPTAR) {
                int sel=SendMessage(hwndLista, LB_GETCURSEL, 0, 0); // LB_GETCURSEL es para ListBox
                if (sel != LB_ERR) {
                    pcap_if_t *d = (pcap_if_t*)SendMessage(hwndLista, LB_GETITEMDATA, sel, 0);
                    char error_buffer[PCAP_ERRBUF_SIZE];
                    
                    resultado.capdev=pcap_open_live(d->name, 65536, 1, 1000, error_buffer);  //guarfar resultado
                    
                    if (resultado.capdev == NULL) {
                        MessageBox(hwnd, "No se pudo abrir la interfaz", "Error", MB_ICONERROR); 
...
pcap_t* MostrarSelectorInterfaces(HINSTANCE hInstance, int *out_link_length) {
    ...
    diagcl.cbSize = sizeof(WNDCLASSEX);
    ...
    RegisterClassEx(&diagcl);

    HWND hwndDiag = CreateWindowEx(WS_EX_DLGMODALFRAME, "SelectorInterfaces", "Interfaces Disponibles - Npcap", 
